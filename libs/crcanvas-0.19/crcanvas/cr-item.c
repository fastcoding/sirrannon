                ...
                double *x1, double *y1, double *x2, double *y2)
{
        ...
        switch (item->device->anchor) {
                case GTK_ANCHOR_N:
                case GTK_ANCHOR_CENTER:
                case GTK_ANCHOR_S:
                        *x1 = bx1 + item->device->x1;
                        *x2 = bx2 + item->device->x2;
                        break;
                case GTK_ANCHOR_NE:
                case GTK_ANCHOR_E:
                case GTK_ANCHOR_SE:
                        *x1 = bx2 + item->device->x1;
                        *x2 = bx2 + item->device->x2;
                        break;
                case GTK_ANCHOR_NW:
                case GTK_ANCHOR_W:
...
set_event_coords(GdkEvent *event, double x, double y)
{
...
_remove(CrItem *item, CrItem *child)
{
        ...
        if (link = g_list_find(item->items, child)) {
                item->flags |= CR_ITEM_NEED_UPDATE;
